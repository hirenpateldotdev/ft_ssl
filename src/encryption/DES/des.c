...
int		g_ip_2[] = {58, 50, 42, 34, 26, 18, 10, 2,
					...
					unsigned long long r, unsigned long long *subkeys, int i)
{
	...
	while (i++ < 16)
	{
		...
		tmp_l = l;
		l = r;
		r = permutate_choice_2(r);//permutate(r, g_e_2, 48, 32);
					...
					unsigned long long r, unsigned long long *subkeys, int i)
{
	...
	while (--i >= 0)
	{
		tmp_r = r;
		r = l;
		l =  permutate_choice_2(l);//permutate(l, g_e_2, 48, 32);
...
static unsigned long long	crypt_des(t_ssl *ssl, unsigned long long block_s)
{
	...
	tmp = block_s;
	block_s = permutate_choice_1(block_s);//permutate(block_s, g_ip_2, 64, 64);//permutate_choice_1
	if (ssl->encrypt)
